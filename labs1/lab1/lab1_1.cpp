...
int main()
{
 ...
 //**********************************************************
	...
	float  f = 0.1234567890123456789123456789; //4b
...
// **************************************************************
	...
	iInt=1;
	double dDouble1=double(iInt/3);		// (1)
	double dDouble2=double(iInt)/3.;	// (2)
	...
	short sShort;
	dDouble=fFloat=iInt=sShort=(cByte=3.3/3);			// (1) int to float - lost precision
	...
	iInt	=	257;
	cByte	=	iInt; // потеря значения 4->1b

	unsigned char cN1=255, cN2=2, cSum;
	cSum = cN1 + cN2; //не влезет
	...
	// Напишите явно преобразования, которые неявно выполняет компилятор
	int iSum = cN1+cN2; // u char to s int
